      ...
      MODELO.find(filtro).lean()
        ...
        .then((registros) => {
          if (registros.length < 1) {
            ...
            resolve({
              ok: true, mensaje: coleccion, contar: registros.length, datos: registros
            })
          }
        })
        .catch((err) => {
...
function nuevo (reg, coleccion, { tipo = 'log', mensaje = '', accion = 'nuevo' } = {}) {
  const promesa = async (resolve, reject) => {
    try {
      const MODELO = coleccion
...
function modificarUno (filtro, reg, coleccion, { tipo = 'log', mensaje = '', accion = 'modificar' } = {}) {
  const promesa = async (resolve, reject) => {
    try {
      ...
      if (!registro) {
        reject({
          ok: false, message: `${coleccion}. Modificar: No se encuentran registros en la búsqueda${JSON.stringify(filtro).replace(/\"/g, '')}`, coleccion, accion: 'modificar'
        })
      } else {
