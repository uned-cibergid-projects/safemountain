...
from tools.androguard4 import apk
from sources import aapt
'''
...
def aapt_parse(app_dict):
    ...
    try:
        log_("info", logger, 'Extracting APK features using aapt/aapt2')
        ...
        log_("info", logger, 'aapt and aapt2 not found, skipping APK feature extraction')

    except Exception as exp:
        log_("info", logger, 'Failed to extract APK features using aapt/aapt2')
...
def androguard_parse(app_dict):
    ...
    try:
        ...
        if not a:
            log_("error", logger, 'Failed to parse APK with androguard')
            return
        ...
        try:
            ...
            log_("error", logger, 'Failed to get app name with androguard')

        try:
            app_dict['androguard_apk_icon'] = a.get_app_icon(max_dpi=0xFFFE - 1)
        except Exception as exp:
            log_("error", logger, 'Failed to get app icon with androguard')

        try:
            xml = a.get_android_manifest_axml().get_xml()
            app_dict['androguard_manifest_xml'] = xml
        except Exception as exp:
            log_("error", logger, 'Failed to parse AndroidManifest.xml with androguard')

        try:
            app_dict['androguard_apk_resources'] = a.get_android_resources()
        except Exception as exp:
            log_("error", logger, 'Failed to parse resources with androguard')

    except Exception as exp:
        log_("exception", logger, 'Failed to parse APK with androguard')
...
def get_apk_name(app_dic):
        ...
        if app_name:
            ...
            if values_path.exists():
                try:
                    ...
                    log_("error", logger, 'Failed to get app name from values folder')
    ...
    # Check if it's source code and try to retrieve the app name
        ...
        try:
            ...
            log_("error", logger, 'Failed to get app name from source code')

    if not real_name:
        #logger.warning('Cannot find app name')
        log_("warning", logger, 'Cannot find app name')
